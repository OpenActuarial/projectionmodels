...
from projectionmodels import BookProjection, GroupProjection, __version__, new_business
...
def run_example() -> dict[str, object]:
    ...
    print("quarterly claims: "
          + "  ".join(f"Q{q+1} ${book.monthly['claims'].to_numpy()[q*3:q*3+3].sum():,.0f}" for q in range(4)))

    return {
        "by_group": book.by_group,
        "monthly": book.monthly,
        "book_premium": book.premium,
        "book_claims": book.claims,
        "book_loss_ratio": book.loss_ratio,
    }
